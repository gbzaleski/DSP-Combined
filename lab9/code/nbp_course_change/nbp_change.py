...
def get_courses(currency_code, days) -> ([float], str):
    ...
    # MonkeyPatch.setattr(requests.get, 'get', mock_get)

    req = requests.get(
        f"http://api.nbp.pl/api/exchangerates/rates/a/{currency_code}/last/{days}/?format=json"
    )
    data = req.json()
    res = [x["mid"] for x in data["rates"]], data["currency"]
    # print(data)
    # print(res)
    return res


# Return days-long list of currency prices and
def mock_get_courses(currency_code, days) -> ([float], str):
